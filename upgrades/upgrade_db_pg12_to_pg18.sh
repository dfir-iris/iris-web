...
ensure_no_iris_db_running() {
    ...
    if docker ps -a --format '{{.Names}}' | grep -qx 'iriswebapp_db'; then
        die "iriswebapp_db container exists (stopped) and still holds the volume. Run \`docker compose down\` (not just stop) before retrying."
    fi
}
