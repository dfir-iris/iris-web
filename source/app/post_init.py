    ...
    if os.getenv("IRIS_WORKER") is None:
        ...
        with app.app_context():
            ...
            if not prevent_objects:
                log.info("Creating base IOC and Artifact types")
...
def create_safe_hooks():
    ...
    # --- artifacts
    create_safe(db.session, IrisHook, hook_name='on_preload_artifact_create', hook_description='Triggered on artifact creation, before commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_postload_artifact_create', hook_description='Triggered on artifact creation, after commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_preload_artifact_update', hook_description='Triggered on artifact update, before commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_postload_artifact_update', hook_description='Triggered on artifact update, after commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_preload_artifact_delete', hook_description='Triggered on artifact deletion, before commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_postload_artifact_delete', hook_description='Triggered on artifact deletion, after commit in DB')
    create_safe(db.session, IrisHook, hook_name='on_manual_trigger_artifact', hook_description='Triggered upon user action')
...
def create_safe_case_states():
    ...
    create_safe(db.session, CaseState, state_name='Reopened', state_description="Case is reopened", protected=True)
...
def create_safe_attributes():
    ...
    create_safe_attr(db.session, attribute_display_name='IOC',
                     ...
                     attribute_content={})
    create_safe_attr(db.session, attribute_display_name='Artifact',
