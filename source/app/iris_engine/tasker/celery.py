...
def _patch_celery_cert_loading():
    ...
    from celery.security import utils
    from cryptography import x509
    
    _original_init = certificate.Certificate.__init__
    _original_load_pem = utils.load_pem_x509_certificate
    
    def _patched_load_pem_x509_certificate(data, password=None):
        """Fixed loader that handles newer cryptography library."""
        if isinstance(data, str):
            data = data.encode('utf-8')
        return x509.load_pem_x509_certificate(data)
    
    utils.load_pem_x509_certificate = _patched_load_pem_x509_certificate
