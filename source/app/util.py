...
def _set_caseid_from_current_user():
    redir = False
    if iris_current_user.ctx_case is None:
        redir = True
        iris_current_user.ctx_case = 1
    caseid = iris_current_user.ctx_case
...
def ac_api_return_access_denied(caseid: int = None):
    error_uuid = uuid.uuid4()
    log.warning(f"EID {error_uuid} - Access denied with case #{caseid} for user ID {iris_current_user.id} "
